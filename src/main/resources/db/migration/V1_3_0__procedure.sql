create procedure bridge.prc_create_meta_by_tag(a_group_tag text, a_meta_tag text)
    language plpgsql
as
$$
declare
...
begin
    ...
    EXECUTE format('COMMENT ON COLUMN %s.s_status is ''Status: 0: nothing happend; 1: successfull processing; -3: processing error (repeat); 4: filtered finally; -4: filtered (repeat); 5: skiped without repeat; 3: critical error''', l_meta_data.raw_full_name);

    EXECUTE format('CREATE INDEX %s_index
        ON %s (s_action)', l_meta_data.raw_name, l_meta_data.raw_full_name);

    EXECUTE format('ALTER TABLE %s ADD CONSTRAINT %s_oper_ch CHECK (f_oper IN (0,1))', l_meta_data.raw_full_name,l_meta_data.raw_name);

    EXECUTE format('ALTER TABLE %s ADD CONSTRAINT %s_action_ch CHECK (s_action IN (0,1))', l_meta_data.raw_full_name,l_meta_data.raw_name);

    EXECUTE format('ALTER TABLE %s ADD CONSTRAINT %s_status_ch CHECK (s_action IN (0,1,3,4,5,-3,-4))', l_meta_data.raw_full_name,l_meta_data.raw_name);
    ...
    EXECUTE format('ALTER TABLE %s ADD CONSTRAINT %s_oper_ch CHECK (f_oper IN (0,1))', l_meta_data.buf_full_name,l_meta_data.buf_name);
...
create procedure bridge.prc_drop_meta_by_tag(a_group_tag text, a_meta_tag text)
    language plpgsql
as
$$
declare
